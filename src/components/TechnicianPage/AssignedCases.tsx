...
// const handleAssignClick = (orderId: string, doctorName: string) => {
//   setStudies((prev) =>
//     prev.map((study) =>
//       study.orderId === orderId
//         ? { ...study, assignedTo: doctorName, isAssigned: false } // reset assignment on doctor change
//         : study
//     )
//   );
// };
...
const CompletedStudiesTable: React.FC<CompletedStudiesTableProps> = ({
    ...
    setStudies((prev) =>
      prev.map((study) => {
        if (study.orderId === orderId) {
          // When a doctor is selected, update the name and always set
          // isAssigned to false. This ensures the user must click "Assign"
          // to confirm the choice, even if one was already assigned.
          return { ...study, assignedTo: doctorName, isAssigned: false };
        }
        return study;
      })
    );
  ...
  const handleAssignClick = (orderId: string) => {
    setStudies((prev) =>
      prev.map((study) => {
        // Only assign if a doctor is selected
        if (study.orderId === orderId && study.assignedTo.trim() !== "") {
          return { ...study, isAssigned: true };
        }
        return study;
